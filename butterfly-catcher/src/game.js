...
const INITIAL_BUTTERFLIES = 4;
const SPAWN_ON_DEPOSIT = 2;
const CARD_BASE_THRESHOLD = 50;
const CARD_THRESHOLD_SCALE = 25;
const BUTTERFLY_FLEE_RADIUS = 150;
const FLOWER_HIDE_RADIUS = 60;
const BEE_COUNT = 3;
const MOSQUITO_COUNT = 4;
...
export class ButterflyCatcherGame {
    constructor() {
        ...
        this.state = 'menu';
        this.elapsedTime = 0;
        this.player = { position: { x: 1200, y: 900 }, speed: 220, catchRadius: 55, hp: PLAYER_MAX_HP, maxHp: PLAYER_MAX_HP };
        this.butterflies = [];
        this.caughtButterflies = [];
        this.bees = [];
        this.mosquitoes = [];
        this.score = 0;
        this.cardChoiceThreshold = CARD_BASE_THRESHOLD;
        this.lastCardChoiceAt = 0;
        this.encyclopedia = new Map();
        this.decorations = [];
        this.doubleCurrencyTimer = 0;
        this.researchSpawnBonus = 0;
        this.shopBtn = null;
        this.shopOverlay = null;
        this.skillDebuffs = new Map();
        this.invulnTimer = 0;
        this.update = (deltaTime) => {
            ...
            for (const debuff of this.skillDebuffs.keys()) {
                ...
                if (t <= 0)
                    this.skillDebuffs.delete(debuff);
                else
                    this.skillDebuffs.set(debuff, t);
            }
            this.butterflies.forEach(b => {
                ...
                if (b.scared && !b.hidden) {
                    ...
                    if (flower) {
                        ...
                        if (fd < FLOWER_HIDE_RADIUS) {
                            ...
                            b.velocity.x = (fdx / fd) * 100;
                            b.velocity.y = (fdy / fd) * 100;
                        }
                    }
                }
                if (b.hidden) {
                ...
                if (b.position.x < 20 || b.position.x > WORLD_W - 20)
                    b.velocity.x *= -1;
                if (b.position.y < 20 || b.position.y > WORLD_H - 20)
                    b.velocity.y *= -1;
            ...
            this.bees.forEach(b => {
                ...
                if (dist < b.aggroRange && b.attackTimer === 0) {
                    ...
                    const spd = 140;
                    if (dist > 1) {
                        b.velocity.x = (pdx / dist) * spd;
                        b.velocity.y = (pdy / dist) * spd;
                    }
                    ...
                    if (b.chasing) {
                        ...
                        b.velocity.x = cd > 1 ? (cdx / cd) * 50 : 0;
                        b.velocity.y = cd > 1 ? (cdy / cd) * 50 : 0;
                    }
                    else {
                        const cdx = b.position.x - b.patrolCenter.x, cdy = b.position.y - b.patrolCenter.y;
                        const cd = Math.sqrt(cdx * cdx + cdy * cdy);
                        if (cd > b.patrolRadius) {
                            b.velocity.x += (-cdx / cd) * 20 * deltaTime;
                            b.velocity.y += (-cdy / cd) * 20 * deltaTime;
                        }
                        if (Math.random() < 0.02) {
                            b.velocity.x = (Math.random() - 0.5) * 60;
                            b.velocity.y = (Math.random() - 0.5) * 60;
                        }
                        b.position.x += b.velocity.x * deltaTime;
                        b.position.y += b.velocity.y * deltaTime;
                    }
            ...
            this.mosquitoes.forEach(m => {
                ...
                if (dist < 150 && m.biteCooldown <= 0) {
                    const spd = 80;
                    if (dist > 1) {
                        m.velocity.x = (pdx / dist) * spd + (Math.random() - 0.5) * 30;
                        m.velocity.y = (pdy / dist) * spd + (Math.random() - 0.5) * 30;
                    }
                    if (dist < 15) {
                    ...
                    if (m.biteCooldown > 0)
                        m.biteCooldown -= deltaTime;
                    if (Math.random() < 0.03) {
                        m.velocity.x = (Math.random() - 0.5) * 50;
                        m.velocity.y = (Math.random() - 0.5) * 50;
                    }
                    m.position.x += m.velocity.x * deltaTime;
                    m.position.y += m.velocity.y * deltaTime;
                }
                if (m.position.x < 20 || m.position.x > WORLD_W - 20)
                    m.velocity.x *= -1;
                if (m.position.y < 20 || m.position.y > WORLD_H - 20)
                    m.velocity.y *= -1;
            ...
            if (atHouse && this.caughtButterflies.length > 0) {
                ...
                if (this.score - this.lastCardChoiceAt >= this.cardChoiceThreshold) {
                    ...
                    this.cardChoiceThreshold += CARD_THRESHOLD_SCALE;
                    this.triggerCardChoice();
                }
                this.spawnButterflies(SPAWN_ON_DEPOSIT);
            }
            if (this.doubleCurrencyTimer > 0) {
                this.doubleCurrencyTimer -= deltaTime;
                if (this.doubleCurrencyTimer < 0)
                    this.doubleCurrencyTimer = 0;
            }
            ...
            if (active && active.completed) {
                if (active.id === 'butterfly_spawn_rate') {
                    this.researchSpawnBonus += 3;
                    this.spawnButterflies(3);
                }
                else if (active.id === 'speed_research')
                    this.skills.upgradeSkill('speed');
                else if (active.id === 'catch_research')
                    this.skills.upgradeSkill('catch_radius');
            }
        ...
        this.render = (_dt) => {
            ...
            if (this.state === 'menu') {
                ...
                this.renderer.drawText('Select from menu', cw / 2, ch / 2 + 40, '#a0aec0', 16);
                return;
            }
            const scale = vh / ch;
            ...
            this.decorations.forEach(d => {
                if (d.type === 'tree')
                    this.renderer.drawTree(d.position.x, d.position.y, d.size);
                else if (d.type === 'flower')
                    this.renderer.drawFlower(d.position.x, d.position.y, d.color, d.size);
                else if (d.type === 'rock')
                    this.renderer.drawRock(d.position.x, d.position.y, d.size);
                else if (d.type === 'bush')
                    this.renderer.drawBush(d.position.x, d.position.y, d.size);
            });
            this.renderer.drawRect(20, 20, 110, 110, '#8b4513');
            this.renderer.drawRect(30, 30, 90, 25, '#a0522d');
            this.renderer.drawText('HOUSE', 50, 48, 'white', 14);
            ...
            this.bees.forEach(b => {
                ...
                this.renderer.drawCircle(b.position.x + 3, b.position.y - 4, 4, '#ecc94b');
                if (b.chasing)
                    this.renderer.drawCircle(b.position.x, b.position.y, 18, 'rgba(246, 173, 85, 0.15)');
            });
            this.mosquitoes.forEach(m => {
                ctx.globalAlpha = 0.6;
            ...
            sl.forEach((s, i) => {
                ...
                const debuff = (this.skillDebuffs.get(s.id) ?? 0) > 0;
            ...
            if (a) {
                this.renderer.drawRect(rx, 126, 120, 12, '#4a5568');
                this.renderer.drawRect(rx, 126, 120 * a.progress, 12, '#4299e1');
                this.renderer.drawText(`${a.name} ${Math.floor(a.progress * 100)}%`, rx + 2, 135, 'white', 9);
            }
            else {
                const allR = this.research.getAllResearch();
                const d = allR.filter(r => r.completed).length;
                this.renderer.drawText(d < allR.length ? `Research: ${d}/${allR.length}` : 'All research done!', rx, 126, d < allR.length ? '#a0aec0' : '#48bb78', 11);
            }
            if (this.doubleCurrencyTimer > 0)
                this.renderer.drawText(`2x ${Math.ceil(this.doubleCurrencyTimer)}s`, 8, 100, '#f9ca24', 12);
        };
    ...
    generateDecorations() {
        const a = () => Math.random();
        for (let i = 0; i < 25; i++)
            this.decorations.push({ type: 'tree', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 35 + a() * 25 });
        const fc = ['#ff6b6b', '#ffa502', '#ff4757', '#e056fd', '#ff9ff3'];
        for (let i = 0; i < 40; i++) {
            const bx = a() * WORLD_W, by = a() * WORLD_H;
            for (let j = 0; j < 3 + Math.floor(a() * 3); j++)
                this.decorations.push({ type: 'flower', position: { x: bx + (a() - 0.5) * 30, y: by + (a() - 0.5) * 30 }, color: fc[Math.floor(a() * fc.length)], size: 4 + a() * 4 });
        }
        for (let i = 0; i < 15; i++)
            this.decorations.push({ type: 'rock', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 10 + a() * 15 });
        for (let i = 0; i < 12; i++)
            this.decorations.push({ type: 'bush', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 20 + a() * 15 });
    }
    findNearestFlower(pos) {
        let best = null;
        let bestD = Infinity;
    ...
    start() { this.dpad.hide(); this.adReward.createButton(); this.adReward.hide(); this.menu.showMenu(); this.gameLoop.start(); }
    startGame() {
        this.state = 'playing';
        this.elapsedTime = 0;
    ...
    trackEncyclopedia(b) {
        ...
        if (e) {
            ...
            this.encyclopedia.set(b.type, { type: b.type, color: b.color, count: 1, totalValue: b.value });
    }
    getEffectiveSkill(id) { return Math.max(1, (this.skills.getSkillLevel(id) - ((this.skillDebuffs.get(id) ?? 0) > 0 ? 1 : 0))); }
    createShopButton() {
        if (this.shopBtn)
            this.shopBtn.remove();
    ...
    showShop() {
        ...
        title.style.cssText = 'color:#e2e8f0;margin:0 0 20px 0;font-size:clamp(20px,5vw,28px);';
        const grid = document.createElement('div');
        grid.style.cssText = `display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;max-width:500px;width:100%;margin-bottom:20px;`;
        this.shop.getItems().forEach(item => {
            const card = document.createElement('div');
            card.style.cssText = 'background:#2d3748;border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:6px;';
            ...
            c.textContent = `Cost: ${item.cost.amount} ${item.cost.type === 'soft' ? 'Soft' : 'Hard'}`;
            c.style.cssText = `color:${item.cost.type === 'soft' ? '#48bb78' : '#f6ad55'};font-size:13px;`;
            card.appendChild(n);
            card.appendChild(d);
            card.appendChild(c);
            if (item.owned) {
                ...
                btn.textContent = 'Buy';
                btn.style.cssText = 'background:#48bb78;color:white;border:none;border-radius:6px;padding:6px 14px;cursor:pointer;font-size:13px;font-weight:bold;';
                btn.onclick = () => { if (this.shop.purchaseItem(item.id)) {
                    this.applyShopEffect(item.id);
                    this.hideShop();
                    this.showShop();
                }
                else {
                    ...
                    setTimeout(() => { btn.textContent = 'Buy'; btn.style.background = '#48bb78'; }, 1200);
                } };
    ...
    applyShopEffect(id) {
        switch (id) {
            case 'double_currency':
                this.doubleCurrencyTimer = 300;
                break;
            case 'speed_boost_item':
                this.skills.upgradeSkill('speed');
                break;
            case 'catch_boost_item':
                this.skills.upgradeSkill('catch_radius');
                break;
            case 'big_net':
                this.player.catchRadius += 20;
                break;
            case 'hp_potion':
    ...
    respawn() {
        this.player.position = { x: 60, y: 60 };
