...
export class ZombieCatcherGame {
    constructor() {
        ...
        this.state = 'menu';
        this.elapsed = 0;
        this.player = { position: { x: 1200, y: 900 }, speed: 200, hp: PLAYER_HP, maxHp: PLAYER_HP };
        this.zombies = [];
        this.bullets = [];
        this.kills = 0;
        this.score = 0;
        ...
        this.update = (dt) => {
            ...
            this.zombies.forEach(z => {
                ...
                if (dist < 250) {
                    z.state = 'chase';
                    if (dist > 1) {
                        z.velocity.x = (pdx / dist) * z.speed;
                        z.velocity.y = (pdy / dist) * z.speed;
                    }
                    ...
                    z.state = 'idle';
                    if (Math.random() < 0.02) {
                        z.velocity.x = (Math.random() - 0.5) * 40;
                        z.velocity.y = (Math.random() - 0.5) * 40;
                    }
                }
                z.position.x += z.velocity.x * dt;
                z.position.y += z.velocity.y * dt;
                ...
                this.zombies.forEach(z => {
                    ...
                    if (dist < z.size + 5) {
                        ...
                        if (z.hp <= 0) {
                            z.killed = true;
                            this.kills++;
                            const val = 5 + Math.floor(z.maxHp / 5);
                            this.currencies.addSoft(val);
                            this.score += val;
                        }
                    }
                });
            ...
            if (this.spawnTimer >= ZOMBIE_SPAWN_RATE && this.zombies.length < MAX_ZOMBIES) {
                this.spawnTimer = 0;
                this.spawnZombies(1 + Math.floor(this.elapsed / 90));
            }
            if (this.player.hp < this.player.maxHp && atHouse)
            ...
            if (act && act.completed) {
                ...
                else if (act.id === 'dmg_res')
                    this.skills.upgradeSkill('damage');
                else if (act.id === 'hp_res')
                    this.skills.upgradeSkill('hp');
            }
        ...
        this.render = (_) => {
            ...
            for (let gy = 0; gy < WORLD_H; gy += 80)
                for (let gx = 0; gx < WORLD_W; gx += 80) {
                    ctx.fillStyle = (Math.floor(gx / 80) + Math.floor(gy / 80)) % 2 === 0 ? '#2d3748' : '#1a202c';
            ...
            this.decorations.forEach(d => {
                if (d.type === 'tree') {
                    ...
                    this.renderer.drawRect(d.position.x, d.position.y, d.size, d.size * 0.8, d.color);
                    this.renderer.drawRect(d.position.x + d.size * 0.3, d.position.y + d.size * 0.2, d.size * 0.15, d.size * 0.15, '#f6e05e');
                }
                else if (d.type === 'fence') {
                    this.renderer.drawRect(d.position.x, d.position.y, d.size, d.size * 0.3, d.color);
                    for (let i = 0; i < 4; i++)
                        this.renderer.drawRect(d.position.x + i * d.size / 4, d.position.y - d.size * 0.3, 3, d.size * 0.3, d.color);
                }
                else if (d.type === 'car') {
                    this.renderer.drawRect(d.position.x, d.position.y, d.size * 2.2, d.size, d.color);
                    this.renderer.drawCircle(d.position.x + d.size * 0.3, d.position.y + d.size, d.size * 0.3, '#1a202c');
                    this.renderer.drawCircle(d.position.x + d.size * 1.7, d.position.y + d.size, d.size * 0.3, '#1a202c');
                }
            });
            this.renderer.drawRect(20, 20, 110, 110, '#4a5568');
            this.renderer.drawRect(30, 30, 90, 25, '#718096');
            this.renderer.drawText('SAFE ZONE', 35, 48, 'white', 13);
            this.renderer.drawRect(30, 65, 90, 55, '#2d3748');
            this.zombies.forEach(z => {
                ...
                if (z.hp < z.maxHp) {
                    const bw = z.size * 1.5;
                    this.renderer.drawRect(z.position.x - bw / 2, z.position.y - z.size - 8, bw, 4, '#4a5568');
                    this.renderer.drawRect(z.position.x - bw / 2, z.position.y - z.size - 8, bw * (z.hp / z.maxHp), 4, '#48bb78');
                }
                if (z.state === 'chase')
                    this.renderer.drawCircle(z.position.x, z.position.y, z.size + 8, 'rgba(72, 187, 120, 0.1)');
            });
            ...
            if (a) {
                ...
                this.renderer.drawText(d < allR.length ? `R&D: ${d}/${allR.length}` : 'All done!', rx, 126, d < allR.length ? '#a0aec0' : '#48bb78', 11);
            }
    ...
    genDecorations() {
        ...
        for (let i = 0; i < 12; i++)
            this.decorations.push({ type: 'building', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 40 + a() * 50, color: colors[Math.floor(a() * colors.length)] });
        for (let i = 0; i < 20; i++)
            this.decorations.push({ type: 'tree', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 30 + a() * 30, color: '#2d5016' });
        for (let i = 0; i < 10; i++)
            this.decorations.push({ type: 'fence', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 15 + a() * 10, color: '#8b7355' });
        for (let i = 0; i < 6; i++)
            this.decorations.push({ type: 'car', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 20 + a() * 15, color: ['#e53e3e', '#3182ce', '#48bb78', '#d69e2e'][Math.floor(a() * 4)] });
    }
    setupData() {
    ...
    menuRender() {
        ...
        t.textContent = '🧟 Zombie Hunter';
        t.style.cssText = 'color:#e2e8f0;margin:0 0 8px;font-size:clamp(24px,7vw,42px);';
        const s = document.createElement('p');
        s.textContent = 'Shoot zombies, survive, earn rewards!';
        s.style.cssText = 'color:#a0aec0;margin:0 0 40px;font-size:clamp(12px,3vw,16px);';
        const btn = document.createElement('button');
        btn.textContent = 'Start Game';
        btn.style.cssText = 'width:min(280px,80vw);padding:16px 32px;border:none;border-radius:12px;font-size:clamp(16px,4vw,20px);cursor:pointer;background:#e53e3e;color:white;font-weight:bold;';
    ...
    showShop() {
        ...
        this.shop.getItems().forEach(item => {
            ...
            co.textContent = `Cost: ${item.cost.amount} ${item.cost.type === 'soft' ? 'Soft' : 'Hard'}`;
    ...
    applyShop(id) {
        if (id === 'medkit')
            ...
            this.zombies = [];
            this.currencies.addSoft(this.kills * 5);
        }
    }
    respawn() {
