...
const CARD_BASE = 80; const CARD_SCALE = 40;
const MAX_FISH = 12; const INITIAL_FISH = 5;

interface Fish { id: string; typeIdx: number; position: Vector2; velocity: Vector2; size: number; value: number; color: string; speed: number; caught: boolean; }
interface Player { position: Vector2; speed: number; netRadius: number; }
interface Decoration { type: 'lilypad' | 'rock' | 'buoy' | 'island'; position: Vector2; size: number; color: string; }
...
export class FishingGame {
  ...
  private player: Player = { position: { x: 1200, y: 900 }, speed: 180, netRadius: 40 };
  private fish: Fish[] = []; private caughtFish: Fish[] = [];
  private score: number = 0; private catches: number = 0;
  ...
  private genDecorations(): void {
    const a = () => Math.random();
    for (let i = 0; i < 30; i++) this.decorations.push({ type: 'lilypad', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 12 + a() * 15, color: '#38a169' });
    for (let i = 0; i < 10; i++) this.decorations.push({ type: 'island', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 50 + a() * 80, color: '#68d391' });
    for (let i = 0; i < 15; i++) this.decorations.push({ type: 'rock', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 8 + a() * 12, color: '#718096' });
    for (let i = 0; i < 8; i++) this.decorations.push({ type: 'buoy', position: { x: a() * WORLD_W, y: a() * WORLD_H }, size: 6, color: '#e53e3e' });
  }
  ...
  private menuRender(): void {
    ...
    const t = document.createElement('h1'); t.textContent = '🎣 Fishing'; t.style.cssText = 'color:#e2e8f0;margin:0 0 8px;font-size:clamp(24px,7vw,42px);';
    const s = document.createElement('p'); s.textContent = 'Catch fish, sell at the dock, upgrade your gear!'; s.style.cssText = 'color:#a0aec0;margin:0 0 40px;font-size:clamp(12px,3vw,16px);text-align:center;';
    const btn = document.createElement('button'); btn.textContent = 'Start Fishing!'; btn.style.cssText = 'width:min(280px,80vw);padding:16px 32px;border:none;border-radius:12px;font-size:clamp(16px,4vw,20px);cursor:pointer;background:#4299e1;color:white;font-weight:bold;';
  ...
  private playGame(): void {
    ...
    this.fish = []; this.caughtFish = []; this.score = 0; this.catches = 0;
    this.lastCardAt = 0; this.cardThreshold = CARD_BASE;
  ...
  private createShop(): void {
    ...
    this.shopBtn = document.createElement('button'); this.shopBtn.className = 'game-ui';
    this.shopBtn.textContent = 'Shop'; this.shopBtn.style.cssText = `position:fixed;bottom:${Math.max(30, window.innerHeight * 0.03)}px;right:${Math.max(20, window.innerWidth * 0.03)}px;padding:10px 16px;background:#4299e1;color:white;border:none;border-radius:8px;font-size:14px;cursor:pointer;z-index:200;font-weight:bold;`;
  ...
  private showShop(): void {
    ...
    const t = document.createElement('h2'); t.textContent = 'Tackle Shop'; t.style.cssText = 'color:#e2e8f0;margin:0 0 20px;font-size:clamp(20px,5vw,28px);';
    ...
    this.shop.getItems().forEach(item => {
      ...
      const co = document.createElement('div'); co.textContent = `Cost: ${item.cost.amount} ${item.cost.type}`; co.style.cssText = `color:${item.cost.type === 'soft' ? '#48bb78' : '#f6ad55'};font-size:13px;`;
  ...
  private applyShop(id: string): void {
    ...
    else if (id === 'speed_up') this.skills.upgradeSkill('boat_speed');
    else if (id === 'net_up') this.skills.upgradeSkill('net_size');
    else if (id === 'sonar') { }
  }
  ...
  private update = (dt: number): void => {
    ...
    if (this.input.mouseDown) {
      ...
      this.fish.forEach(f => {
        ...
        if (dist < nr) { f.caught = true; this.caughtFish.push(f); }
      });
    }

    this.fish.forEach(f => {
      if (Math.random() < 0.02) { f.velocity.x = (Math.random() - 0.5) * f.speed; f.velocity.y = (Math.random() - 0.5) * f.speed; }
      f.position.x += f.velocity.x * dt; f.position.y += f.velocity.y * dt;
      if (f.position.x < 20 || f.position.x > WORLD_W - 20) f.velocity.x *= -1;
      if (f.position.y < 20 || f.position.y > WORLD_H - 20) f.velocity.y *= -1;
    ...
    if (atDock && this.caughtFish.length > 0) {
      ...
      this.currencies.addSoft(total); this.score += total; this.catches += this.caughtFish.length;
      this.caughtFish = [];
    ...
    if (a && a.completed) {
      if (a.id === 'speed_r') this.skills.upgradeSkill('boat_speed');
      else if (a.id === 'net_r') this.skills.upgradeSkill('net_size');
    }
  ...
  private render = (_: number): void => {
    ...
    ctx.fillStyle = '#1a3a5c'; ctx.fillRect(0, 0, WORLD_W, WORLD_H);
    for (let gy = 0; gy < WORLD_H; gy += 80) for (let gx = 0; gx < WORLD_W; gx += 80) { ctx.fillStyle = (Math.floor(gx / 80) + Math.floor(gy / 80)) % 2 === 0 ? '#1e4d7a' : '#1a3a5c'; ctx.fillRect(gx, gy, 80, 80); }

    this.decorations.forEach(d => {
      if (d.type === 'lilypad') { ctx.beginPath(); ctx.ellipse(d.position.x, d.position.y, d.size, d.size * 0.6, 0, 0, Math.PI * 2); ctx.fillStyle = d.color; ctx.fill(); }
      else if (d.type === 'island') { ctx.beginPath(); ctx.ellipse(d.position.x, d.position.y, d.size, d.size * 0.7, 0, 0, Math.PI * 2); ctx.fillStyle = d.color; ctx.fill(); ctx.beginPath(); ctx.ellipse(d.position.x - d.size * 0.2, d.position.y - d.size * 0.3, d.size * 0.3, d.size * 0.4, 0, 0, Math.PI * 2); ctx.fillStyle = '#2f6b2f'; ctx.fill(); }
      else if (d.type === 'rock') { ctx.beginPath(); ctx.ellipse(d.position.x, d.position.y, d.size, d.size * 0.6, 0, 0, Math.PI * 2); ctx.fillStyle = d.color; ctx.fill(); }
      else if (d.type === 'buoy') { this.renderer.drawRect(d.position.x - 2, d.position.y - 10, 4, 12, '#4a5568'); this.renderer.drawCircle(d.position.x, d.position.y, d.size, d.color); }
    });

    this.renderer.drawRect(20, 20, 110, 110, '#8b4513'); this.renderer.drawRect(30, 30, 90, 25, '#a0522d'); this.renderer.drawText('DOCK', 50, 48, 'white', 14); this.renderer.drawRect(30, 65, 90, 55, '#6b3410');
    ...
    const a = this.research.getActiveResearch();
    if (a) { this.renderer.drawRect(rx, 100, 120, 12, '#4a5568'); this.renderer.drawRect(rx, 100, 120 * a.progress, 12, '#4299e1'); this.renderer.drawText(`${a.name} ${Math.floor(a.progress * 100)}%`, rx + 2, 109, 'white', 9); }
    else { const allR = this.research.getAllResearch(); const d = allR.filter(r => r.completed).length; this.renderer.drawText(d < allR.length ? `R&D: ${d}/${allR.length}` : 'All done!', rx, 100, d < allR.length ? '#a0aec0' : '#48bb78', 11); }
